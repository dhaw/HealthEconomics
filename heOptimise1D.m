...
ub=ones(numInt,1);
X0=repmat(data1.xmin,numInt,1);%zeros(lx,1);
Xit=repmat(data1.xmin,numInt,1);
%%
fun1=@(Xit)econGDP(Xit);
%fun2=@(pmod)epiConstraint(pr,n,nbar,na,NN,NNbar,NNrep,Din,beta,Xit,tvec,hospThresh,data1,pmod);
nonlcon=@(pmod)epiConstraint(pr,n,nbar,na,NN,NNbar,NNrep,Din,beta,X0,tvec,hospThresh,data1);
%{
