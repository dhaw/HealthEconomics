function [pr,NN,n,nbar,na,NNbar,NNrep,Dout,beta]=hePrepCovid19Old(D)%,hvec,muvec)
...
hvec=[.04,.04,.278,.599]';
muvec=[.0025,.0025,.0108,.1068]';
gamma4=[];
%
hvec=[repmat(hvec(adInd),lx,1);hvec];
muvec=[repmat(muvec(adInd),lx,1);muvec];
...
pr.h=hvec;%ph./(1-ph);%1/toHosp;
pr.mu=muvec;%.0025/.04;%muvec;
pr.odds=0;
pr.qnew=0;
pr.red=2/3;
pr.R0=2.5;
